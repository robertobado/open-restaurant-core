...
public class BaseResourceTest {
	...
	private static WebServerEnvironment createWebServerEnvironment() {
		return new JettyWebServerEnvironment(BaseResource.SERVER_HOSTNAME, BaseResource.PORT_NUMBER,
				BaseResource.CONTEXT_PATH, "src/main/webapp");
	}
	...
	protected void testWrongContentTypePost(String path)
		...
		HttpPost httpPost = new HttpPost(getServerBasePath() + BaseResource.SLASH + path);
		...
		if(!StringUtils.isBlank(authorizedToken)){
			Header loginTokenHeader = new BasicHeader(BaseResource.LOGIN_TOKEN_HEADER_PARAMETER_NAME,
					authorizedToken);
	...
	protected void testWrongContentTypeGet(String path)
		...
		HttpGet httpGet = new HttpGet(getServerBasePath() + BaseResource.SLASH + path + BaseResource.SLASH + 1);
	...
	protected HttpGet generateBasicHttpGet(String path) {
		HttpGet httpGet = new HttpGet(getServerBasePath() + BaseResource.SLASH + path);
	...
	protected void testWrongContentTypeDelete(String path)
		...
		HttpDelete httpDelete = new HttpDelete(getServerBasePath() + BaseResource.SLASH + path + BaseResource.SLASH + 1);
	...
	protected HttpDelete generateBasicHttpDelete(String path) {
		HttpDelete httpDelete = new HttpDelete(getServerBasePath() + BaseResource.SLASH + path);
	...
	protected void testWrongContentTypePut(String path)
		...
		HttpPut httpPut = new HttpPut(getServerBasePath() + BaseResource.SLASH + path + BaseResource.SLASH + 1);
	...
	protected HttpPut generateBasicHttpPut(String path) {
		HttpPut httpPut = new HttpPut(getServerBasePath() + BaseResource.SLASH + path);
	...
	protected User createUserAndPersist(Configuration configuration, String randomPassword) {
		User user = new User();
		
		user.setCompany(configuration.getCompany());
		user.setLogin(RandomGenerator.generateString());

		String passwordWithSalt = configuration.getHashSalt() + "/"
				+ randomPassword;
